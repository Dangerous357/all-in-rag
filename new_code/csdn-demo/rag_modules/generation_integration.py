...
class GenerationIntegrationModule:
    ...
    def __init__(self, model_name: str = "qwen-flash", temperature: float = 0.1, max_tokens: int = 2048):
        """
        初始化生成集成模块
...
你是博主RK_Dangerous的CSDN专业博客智能助手。请根据下面提供的技术博客内容，准确、专业、简洁地回答用户问题。
...
请提供详细、实用的回答。如果信息不足，请诚实说明。如果文中出现相关描述，可以基于内容合理推断并回答。
...
回答:""")
    ...
    def generate_step_by_step_answer(self, query: str, context_docs: List[Document]) -> str:
        ...
        prompt = ChatPromptTemplate.from_template("""
你是博主RK_Dangerous的技术博客助手。请根据博客内容，生成专业、结构化、可直接学习的技术回答。
...
回答:""")
    ...
    def query_rewrite(self, query: str) -> str:
        ...
        prompt = PromptTemplate(
            template="""
你是一个智能查询分析助手。请分析用户的查询，判断是否需要重写使其更适合检索CSDN技术文章。

原始查询: {query}

分析规则：
1. **具体明确的查询**（直接返回原查询）：
   - 包含具体的技术名称：如"点云是什么？"、"如何使用PyTorch？"

2. **模糊不清的查询**（需要重写成技术搜索语句）：
   - 过于宽泛：如"讲一下AI？"、“有什么文章？”
...
1. 'list' - 用户要文章推荐
   例如：推荐文章、有哪些博客、写过哪些技术

2. 'detail' - 用户要具体技术/原理/步骤/代码/复现
   例如：CNN怎么实现？代码讲解？梯度如何计算？

3. 'general' - 通用问题/概念/简介/总结
   例如：什么是CSS？池化介绍？
...
分类结果:""")
    ...
    def generate_list_answer(self, query: str, context_docs: List[Document]) -> str:
        ...
        """
        if not context_docs:
            return "抱歉，没有找到相关的文章信息。"

        # 提取文章名称
        dish_names = []
        for doc in context_docs:
            dish_name = doc.metadata.get('dish_name', '未知文章')
